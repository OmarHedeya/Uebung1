...
public class SecondTest {
	...
	@Test
	public void resizeOnePictureAndOverwrite(){
		try {
			image = ImageIO.read(filePicture);
		} catch (IOException e1) {
          assertTrue("Image not Found", false);
		...
		int heightToBe = (int)Math.floor((double)image.getHeight()/2.0);
		try {
			test.generateImage(filePicture,outputDir , true, widthToBe,heightToBe ,"resizePortrait2");
		} catch (IOException e) {
			assertTrue(false);
		}
		File outputImage = new File("target//data_test//resizePortrait2" +filePicture.getName());
		try {
			image2 = ImageIO.read(outputImage);
		} catch (IOException e) {
           assertTrue(false);
		...
		try {
		 ...
		 widthToBe = (int)Math.floor((double)image.getWidth()/2.0);
		 heightToBe = (int)Math.floor((double)image.getHeight()/2.0);
		try {
			test.generateImage(outputImage,outputDir , true, widthToBe,heightToBe ,""); //filePic instead of out in first?
		} catch (IOException e) {
			assertTrue(false);
		}
		 outputImage = new File("target//data_test//resizePortrait2" +filePicture.getName());
		try {
			image2 = ImageIO.read(outputImage);
		} catch (IOException e) {
           assertTrue(false);
		}
		 outWidth = image2.getWidth();
		 outHeight = image2.getHeight();
		assertTrue("The image could not be resized to these dimensions",(widthToBe == outWidth || widthToBe == outWidth - 1 || widthToBe == outWidth +1 ) && (heightToBe == outHeight || heightToBe == outHeight -1 || heightToBe == outHeight +1));
	}	
